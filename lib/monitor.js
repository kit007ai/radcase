 ...
 * Tracks request metrics, response times, cache performance, memory usage
 */
...
class PerformanceMonitor {
  ...
  middleware() {
    return (req, res, next) => {
      ...
      res.on('finish', () => {
        ...
        if (status >= 400) this.requests.errors++;
  ...
  getHealth() {
    ...
    const issues = [];
    if (heapUsedPct > 90) issues.push('High heap usage: ' + heapUsedPct.toFixed(0) + '%');
    ...
    return {
      ...
      issues,
      checks: {
        memory: heapUsedPct < 90 ? 'ok' : 'warning',
